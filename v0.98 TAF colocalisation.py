...
root = 'D:\\Ed\\20190418 54BP1 H2AX coloc\\' #remember to use double backslash or single forward slash (is a requirement for python convention)
input_file_H2AX = root + '20190417 H2AX.xlsx'
input_file_TELO = root + '20190417 53BP1.xlsx'
input_file_DAPI = root + '20190417 DAPI.xlsx'
...
TAF_positive_threshold = float(5) #input("Number of TAF to qualify as senescence-positive: \n")
...
maxint_TELO = np.array(df_TELO[20])
avint_TELO = np.array(df_TELO[19])
...
def full_analysis(index_1,index_2,index_3,index_4,index_5,index_6):
    ...
    for (Tkey, Tval), (Hkey, Hval) in zip(dict_nuclei_TELO.items(),dict_nuclei_H2AX.items()):
        TAF_TELO,TAF_H2AX,TELO_length,_53BP1_total,H2AX_total = [],[],[],[],[]
        if Hkey == Tkey:
            for Tval2 in Tval:
                _53BP1_total.append(Tval2[0:3])
                for Hval2 in Hval:
        ...
        if len(TAF_TELO) or len(_53BP1_total) > 0:
            percent_53BP1[Tkey] = (len(TAF_TELO) / len(_53BP1_total)) * 100
        if len(TAF_TELO) or len(H2AX_total) > 0:
            percent_H2AX_vs_53BP1[Tkey] = (len(TAF_TELO) / len(H2AX_total)) * 100
        TTAF[Tkey] = TAF_TELO[:]
...
for n, obj in enumerate(dataset_indices):
    ...
    images_53BP1, images_H2AX_vs_53BP1 = {},{}
    for m, obj_2 in enumerate(image_indices):
        if m > 0 and n <= len(dataset_indices):
            if ((image_indices[m-1][0] >= (dataset_indices[n-1][0])) and
                ...
                images_53BP1[Image_num] = analysis[8]
                images_H2AX_vs_53BP1[Image_num] = analysis[9]
                ...
                percent_53BP1.update({dataset_obj[n-1] : images_53BP1})
                percent_H2AX_vs_53BP1.update({dataset_obj[n-1] : images_H2AX_vs_53BP1})
            else:
                pass
